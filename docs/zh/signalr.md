...
两包均含 **Observables.SignalR** 运行时（`HubService`、`SignalRObservable` 桥接）及对应 Roslyn 分析器。当前从 [Observables](https://github.com/Skymly/Observables) 仓库构建；下一版 NuGet 预览（`0.1.0-preview2` 之后）将按 Events/RestAPI 相同方式发布到 nuget.org。
