...
**`0.1.0`** 已发布至 [nuget.org](https://www.nuget.org/packages/Observables.Events.R3)（**16** 个元包：Events、RestAPI、SignalR、Mqtt、WebSocket、Grpc、Sse）。每个功能域有两个包（R3 与 System.Reactive）：
...
| `Observables.Nats.R3` | Core NATS subject 代理 → R3（规划 `0.1.0`） |
| `Observables.Nats.Reactive` | 同上 → System.Reactive（规划 `0.1.0`） |
