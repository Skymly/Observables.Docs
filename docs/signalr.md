...
Both include the backend-neutral **Observables.SignalR** runtime (`HubService`) and the matching Roslyn analyzer. The `SignalRObservable` R3 bridges ship only in the `.R3` package; the `.Reactive` package carries its System.Reactive adapter instead. **`0.1.1`** and later are on [nuget.org](https://www.nuget.org/packages/Observables.SignalR.R3) (same model as Events/RestAPI).
